...
# --- BASE DE DATOS (Carga desde Google Sheets reemplazando CSV) ---
...
def cargar_datos_flotodo():
    try:
        ...
        # Limpieza de Fecha (Manteniendo lógica original)
        ...
        # Limpieza Tipo Sorteo (Manteniendo lógica original)
        ...
        # Crear df_fijos (Manteniendo lógica original)
...
# --- FUNCIONES DE ESCRITURA (Reemplazan escritura de CSV) ---
...
# --- INICIO CÓDIGO ORIGINAL (Limpieza de funciones auxiliares duplicadas en la fusión) ---
...
def analizar_almanaque(df_fijos, dia_inicio, dia_fin, meses_atras, strict_mode=True):
    ...
    # === ARREGLO: NO incluir el mes actual en los bloques históricos ===
    # El mes actual no ha terminado, así que empezamos desde el mes anterior
    for offset in range(1, meses_atras + 1):
        f_obj = fecha_hoy - relativedelta(months=offset)
        
        # SALTAR si el mes calculado es el mismo que el mes actual (puede pasar por cómo funciona relativedelta)
    ...
    # === ARREGLO: Evaluación del período actual ===
    # Comparar solo fechas (sin hora) para evitar errores cuando hoy es antes del inicio del período
    hoy = datetime.now()
    hoy_date = hoy.date()
    estado_periodo = ""
    ...
    try:
        ...
        if hoy_date < fecha_ini_date:
            ...
            fecha_fin_real = fecha_fin_teorica  # Mes anterior ya terminó
            
            estado_periodo = f"🟡 PERIODO MES ANTERIOR ({fecha_ini_evaluacion.strftime('%d/%m')} - {fecha_fin_teorica.strftime('%d/%m')})"
        else:
            # Período del mes actual está activo
            # Usar la fecha de HOY como límite real para incluir todos los sorteos de hoy
            fecha_fin_real = hoy
...
# === FUNCIÓN NUEVA: Fecha inteligente para agregar sorteo ===
def obtener_fecha_sorteo_inteligente():
    """
    Determina la fecha correcta para agregar sorteo según la hora:
    - Si es antes de las 23:00 (hora Noche), mantener fecha actual
    - Si es después de las 23:00, usar fecha de mañana (el próximo sorteo será mañana)
    """
    ahora = datetime.now()
    hora_actual = ahora.hour
    
    # Hora límite del sorteo Noche (23:00 = 11 PM)
    # Si ya pasó la Noche de hoy, la app sugiere mañana
    if hora_actual >= 23:
                ...
                if new_u and new_p:
                    try:
                        ...
                        if existe:
                            ...
                            # Columnas: Usuario, Password_Hash, Rol, Token_Sesion, Activo
...
with st.sidebar.expander("📝 Agregar Sorteo"):
    # ARREGLO: Usar fecha inteligente según hora
