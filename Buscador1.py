...
def analizar_almanaque(df_fijos, dia_inicio, dia_fin, meses_atras, strict_mode=True):
    ...
    # CORRECCIÓN: Empezar desde offset=1 para NO incluir el mes actual
    for offset in range(1, meses_atras + 1):
        f_obj = fecha_hoy - relativedelta(months=offset)
        
        # Verificación extra: saltar si coincide con mes actual
...
def backtesting_estrategia(df_fijos, mes_objetivo, anio_objetivo, dia_ini, dia_fin, meses_atras):
    try:
        ...
        # CORRECCIÓN: Usar 23:59:59 para incluir TODOS los sorteos del último día (M, T, N)
        f_prueba_fin = datetime(anio_objetivo, mes_objetivo, min(dia_fin, calendar.monthrange(anio_objetivo, mes_objetivo)[1]), 23, 59, 59)
...
with tabs[7]:
    ...
    with col_l3:
        ...
        with c_dia2: dia_fin_lab = st.number_input("Día Fin:", 1, 31, 28, key="lab_dia_fin")
...
st.markdown("---")
st.caption("Geotodo Suite Ultimate v1.2 | 🍑 Adaptado para 3 Sesiones (M, T, N)")