...
RUTA_BACKTEST = 'backtest_detalle_florida.csv'
RUTA_HISTORICO = 'historico_predicciones_florida.csv'
...
def cargar_datos_geotodo(_ruta_csv, _file_signature):
    try:
        ...
        if 'Tipo_Sorteo' in df.columns:
            ...
            df = df[df['Tipo_Sorteo'].isin(['T', 'N'])].copy() # Solo Tarde y Noche
        else:
...
def obtener_historial_perfiles_cacheado(df_full, ruta_cache=None):
    ...
    if not df_cache.empty and 'Fecha' in df_cache.columns and 'Sorteo' in df_cache.columns:
        sorteo_map_inv = {'Tarde': 'T', 'Noche': 'N', 'T': 'T', 'N': 'N'}
...
def main():
    ...
    with st.sidebar.expander("📝 Agregar Sorteo", expanded=False):
        ...
        with col_f2:
            ses = st.radio("Sesión", ["Tarde", "Noche"], horizontal=True, key="sesion_radio_nuevo")
    ...
    modo_sorteo = st.sidebar.radio("Análisis:", ["General", "Tarde", "Noche"], key="modo_sorteo_radio")
    ...
    if modo_fecha == "Personalizado":
        ...
        sesion_estado = st.sidebar.radio("Estado:", ["Antes de Tarde", "Después de Tarde"], horizontal=False, key="sesion_estado_radio")
    ...
    with tabs[0]:
        ...
        # 🔍 CORRECCIÓN APLICADA: Filtro por sesión seleccionada (Solo T/N)
        if modo_sorteo == "Tarde":
        ...
        if not df_hist_perfiles.empty:
            ...
            sort_map = {'Tarde': 0, 'Noche': 1, 'T': 0, 'N': 1}
