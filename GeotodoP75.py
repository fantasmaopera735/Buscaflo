...
def generar_sugerencia_fusionada(df_stats, transizioni, ultimo_perfil, df_oport_dec, df_oport_uni, df_historial_perfiles, fecha_ref, estabilidad_digitos, gaps_dec, gaps_uni, distribuciones_perfiles, freq_dec, freq_uni, faltantes_mes_anterior=None):
    ...
    if not alertas_activas.empty:
        for _, row_alert in alertas_activas.iterrows():
            ...
            with st.container():
                ...
                st.info(f"🔙 **Historia última salida:** Fecha: **{ult_fecha.strftime('%d/%m/%Y')}** | Estado: **{ult_estado}** | Estabilidad: **{ult_estabilidad}%**")
                st.markdown("---")
    else:
    ...
    if lista_nums:
        ...
        for idx, num in enumerate(lista_nums):
            d_int = int(num // 10); u_int = int(num % 10)
            ed = map_estado_dec.get(d_int, "?"); eu = map_estado_uni.get(u_int, "?")
...
def main():
    ...
    # --- BLOQUE RESTAURADO: INFO ULTIMOS SORTEOS ---
    if not df_full.empty:
        sort_map = {'M':0, 'T':1, 'N':2}
        df_last = df_full[df_full['Posicion']=='Fijo'].copy()
        df_last['ord'] = df_last['Tipo_Sorteo'].map(sort_map)
        df_last = df_last.sort_values(['Fecha','ord'])
        ult = df_last.iloc[-1]
        u_t = ult['Tipo_Sorteo']
        u_f = ult['Fecha']
        ...
        iconos = {'M':'🌅', 'T':'☀️', 'N':'🌙'}
        names = {'M':'Mañana', 'T':'Tarde', 'N':'Noche'}
        for t in ['M', 'T', 'N']:
            df_t = df_full[df_full['Tipo_Sorteo']==t]
            if not df_t.empty:
                r = df_t[df_t['Fecha']==df_t['Fecha'].max()].iloc[0]
                st.sidebar.markdown(f"**{iconos[t]} {names[t]} ({r['Fecha'].strftime('%d/%m')})**")
                # Intentar mostrar numero completo
                mask = (df_full['Fecha']==r['Fecha']) & (df_full['Tipo_Sorteo']==t)
                try:
                    cen = df_full.loc[mask & (df_full['Posicion']=='Centena'), 'Numero'].iloc[0]
                    fij = df_full.loc[mask & (df_full['Posicion']=='Fijo'), 'Numero'].iloc[0]
                    c1 = df_full.loc[mask & (df_full['Posicion']=='1er Corrido'), 'Numero'].iloc[0]
                    st.sidebar.markdown(f"Num: **{int(f'{cen}{fij:02d}')}** | C1: `{c1}`")
                except: pass
                st.sidebar.markdown("---")
        
        # Calcular target automatico
    ...
    t_val = ord_s.get(target, 0) # Fix error si target no esta
    ...
    st.markdown("---")
    # --- FIX ERROR KEYERROR: Solo comparar si NO es General ---
    if modo != "General":
