...
st.markdown("Motor con Backtest Real + Dashboard de Efectividad + **Faltantes del Mes**")
...
# ============================================================================
# 🆕 ANALIZAR ESTABILIDAD DE NÚMEROS (PARA FALTANTES)
# ============================================================================
...
# ============================================================================
# 🆕 ANALIZAR FALTANTES DEL MES (ALGORITMO GEOTODO COMPLETO)
# ============================================================================
def analizar_faltantes_mes(df_fijos, mes, anio, sorteos_freq=1000):
    try:
        ...
        df_mes = df_fijos[(df_fijos['Fecha'] >= fecha_inicio_mes) &
                         (df_fijos['Fecha'] <= fecha_fin_mes)]
        ...
        for num in faltantes:
            ...
            # 🎯 CLAVE: Si NO tiene condiciones, recibe 50 pts
...
# ============================================================================
# 🆕 OBTENER FALTANTES DEL MES ACTUAL
# ============================================================================
def obtener_faltantes_mes_actual(df_fijos):
    try:
        hoy = datetime.now()
        fecha_inicio_mes = datetime(hoy.year, hoy.month, 1)
        fecha_fin_mes = hoy
        df_mes = df_fijos[(df_fijos['Fecha'] >= fecha_inicio_mes) &
                         (df_fijos['Fecha'] <= fecha_fin_mes)]
        salidos = set(df_mes['Numero'].unique())
...
# ============================================================================
# 🆕 OBTENER PREDICCION CON BONUS DE FALTANTES (SIN REPETIDOS)
# ============================================================================
def obtener_prediccion_numeros_lista(df_stats, transizioni, ultimo_perfil, df_oport_dec, df_oport_uni, df_historial_perfiles, fecha_ref, estabilidad_digitos, gaps_dec, gaps_uni, distribuciones_perfiles, freq_dec, freq_uni, faltantes_mes_actual=None, debug_mode=False, debug_logs=None):
    if df_stats.empty:
    ...
    # 🆕 SET DE FALTANTES PARA BONUS
    faltantes_set = set(faltantes_mes_actual) if faltantes_mes_actual else set()
    ...
    for _, row in top_7.iterrows():
        ...
        for d in decenas_estado:
            for u in unidades_estado:
                ...
                # 🆕 BONUS DE FALTANTES DEL MES (+50 PTS)
                bonus_faltantes = 0
    ...
    # ✅ VERIFICAR NO HAYA REPETIDOS AL FINAL
...
# ============================================================================
def generar_sugerencia_fusionada(df_stats, transizioni, ultimo_perfil, df_oport_dec, df_oport_uni, df_historial_perfiles, fecha_ref, estabilidad_digitos, gaps_dec, gaps_uni, distribuciones_perfiles, freq_dec, freq_uni, faltantes_mes_actual=None):
    ...
    # ========================================================================
    # 🎲 TOP 40 NÚMEROS SUGERIDOS
    # ========================================================================
    st.markdown("### 🎲 Top 40 Números Sugeridos")
    lista_nums = obtener_prediccion_numeros_lista(df_stats, transizioni, ultimo_perfil, df_oport_dec, df_oport_uni, df_historial_perfiles, fecha_ref, estabilidad_digitos, gaps_dec, gaps_uni, distribuciones_perfiles, freq_dec, freq_uni, faltantes_mes_actual)
    ...
    for idx, num in enumerate(lista_nums):
        try:
            es_faltante = num in (faltantes_mes_actual if faltantes_mes_actual else [])
...
def guardar_prediccion_en_historico(fecha, sorteo, top40, perfil_top, score_promedio, numero_real=None):
    ...
    # Verificar si ya existe predicción pendiente para esta fecha/sorteo
...
# ============================================================================
# 📊 DASHBOARD EFECTIVIDAD (COMPLETO Y FUNCIONAL)
# ============================================================================
...
def ejecutar_backtest(df_full, sorteos_objetivo, nombre_sesion, debug_mode=False):
    ...
    for i in range(sorteos_a_procesar):
        ...
        faltantes_mes = obtener_faltantes_mes_actual(df_historial)
...
# ============================================================================
# 🆕 MOSTRAR ÚLTIMOS RESULTADOS EN SIDEBAR (COMPLETO)
# ============================================================================
def mostrar_ultimos_resultados_sidebar(df_full):
    """Muestra los últimos resultados de cada sesión en el sidebar"""
    st.sidebar.markdown("---")
...
def main():
    ...
    with tabs[0]:
        ...
        # KEY FIJA para botón de análisis
        if st.button("🚀 Ejecutar Análisis", type="primary", key="btn_ejecutar_analisis_principal"):
            with st.spinner("Analizando..."):
                if not df_historial_perfiles_full.empty:
                    ...
                    faltantes_mes_actual = obtener_faltantes_mes_actual(df_analisis)
                    
                    # ✅ GENERAR SUGERENCIA Y OBTENER TOP40
                    top40 = generar_sugerencia_fusionada(
                        ...
                        faltantes_mes_actual
                    )
                    
                    # ✅ GUARDAR PREDICCION SI SE GENERO TOP40
    ...
    with tabs[1]:
        ...
        with col_f3:
            cant_sorteos = st.slider("Sorteos:", 100, 5000, 1000, step=100, key="cant_sorteos_faltantes")
        
        # KEY FIJA para botón de faltantes
        if st.button("🔍 Analizar Faltantes", type="primary", key="btn_analizar_faltantes_tab"):
            with st.spinner("Calculando..."):
                try:
                    df_faltantes_res, error_msg, df_salidos_mes = analizar_faltantes_mes(df_full, mes_num, anio_sel, cant_sorteos)
                    ...
                    if error_msg:
                        ...
                        st.markdown(f"### ⏳ Faltan: {total_faltantes} números")
                        
                        col_stat1, col_stat2, col_stat3 = st.columns(3)
                        col_stat1.metric("🔴 Alta", len(alta))
                        col_stat2.metric("🟡 Media", len(media))
                        col_stat3.metric("⚪ Baja", len(baja))
                        
                        st.markdown("---")
                        st.markdown("#### 🔴 Prioridad Alta")
                        st.write(" ".join([f"`{n}`" for n in alta['Número'].tolist()]))
                        st.markdown("---")
                        st.dataframe(df_faltantes_res, use_container_width=True, hide_index=True)
                    else:
    ...
    with tabs[3]:
        ...
        # KEY FIJA para botón de backtest
