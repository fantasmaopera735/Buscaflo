...
RUTA_CACHE = 'cache_digitos_georgia.csv'

st.set_page_config(
    page_title="Georgia - Análisis de Dígitos P75",
    page_icon="🍑",
    layout="wide",
    initial_sidebar_state="expanded"
)
st.title("🍑 Georgia - Análisis de Dígitos P75 (Decena/Unidad)")
...
if 'distribuciones_digitos' not in st.session_state:
    st.session_state.distribuciones_digitos = None
...
def analizar_estadisticas_digitos(df_fijos, fecha_ref):
    ...
    for tipo_pos in ['Decena', 'Unidad']:
        for digito in range(10):
            # Filtrar fechas donde apareció este dígito en esta posición
            if tipo_pos == 'Decena':
            ...
            # Calcular estados históricos y métricas de última salida
            estados_hist = []
            ...
            for i, g in enumerate(gaps):
                # Calcular P75 dinámico para cada gap histórico
                gaps_prev = gaps[:i]
                ...
                # Guardar métricas de la ÚLTIMA salida (la que cierra el historial)
                if i == len(gaps) - 1:
...
def main():
    ...
    if st.button("🚀 Ejecutar Análisis de Dígitos", type="primary", key="btn_analisis"):
        ...
        with st.spinner("Calculando Gaps, P75 y Estados por dígito..."):
            ...
            mostrar_estadistica_digitos(df_stats)
