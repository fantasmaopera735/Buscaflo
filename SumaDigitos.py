...
ORDEN DE SESIONES (de más reciente a menos reciente): N → T → M
"""
...
def cargar_datos():
    try:
        ...
        # Calcular sumas de dígitos (0-18)
    ...
    try:
        ...
        if hoy.date() < fecha_ini.date():
            ...
            if not df_eval.empty:
                ...
                # ORDEN: N=0 (más reciente), T=1, M=2 (menos reciente) - CORREGIDO
...
def buscar_suma_detalle(df, suma_buscar, tipo_sesion=None):
    ...
    # ORDEN: N=0 (más reciente), T=1, M=2 (menos reciente) - CORREGIDO
    orden_sesion = {'N': 0, 'T': 1, 'M': 2}
    df_resultados['orden'] = df_resultados['Sesión'].map(orden_sesion).fillna(3)
...
# --- FUNCIONES DE COMBINACIONES/TRANSICIONES ---
...
def analizar_transicion_manana_tarde(datos, tipo_suma='Suma_Fijo'):
    """Analiza: Si en la mañana sale X, ¿qué sale en la tarde?"""
    transiciones = Counter()
    
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'T' in sesiones:
            suma_manana = sesiones['M'][tipo_suma]
            suma_tarde = sesiones['T'][tipo_suma]
            transiciones[(suma_manana, suma_tarde)] += 1
    
    return transiciones

def analizar_transicion_manana_tarde_noche(datos, tipo_suma='Suma_Fijo'):
    """Analiza: Si mañana=X y tarde=Y, ¿qué sale en la noche?"""
    transiciones = Counter()
    
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'T' in sesiones and 'N' in sesiones:
            suma_manana = sesiones['M'][tipo_suma]
            suma_tarde = sesiones['T'][tipo_suma]
            suma_noche = sesiones['N'][tipo_suma]
            transiciones[(suma_manana, suma_tarde, suma_noche)] += 1
    
    return transiciones

def analizar_transicion_dia_siguiente(datos, tipo_suma='Suma_Fijo'):
    """Analiza: Si mañana=X, tarde=Y, noche=Z, ¿qué sale en la mañana siguiente?"""
    transiciones = Counter()
    ...
    for i in range(len(fechas_ordenadas) - 1):
        ...
        if all(s in datos[fecha_actual] for s in ['M', 'T', 'N']) and 'M' in datos[fecha_siguiente]:
            suma_m = datos[fecha_actual]['M'][tipo_suma]
            suma_t = datos[fecha_actual]['T'][tipo_suma]
            suma_n = datos[fecha_actual]['N'][tipo_suma]
            suma_m_siguiente = datos[fecha_siguiente]['M'][tipo_suma]
            
            transiciones[(suma_m, suma_t, suma_n, suma_m_siguiente)] += 1
    
    return transiciones

def obtener_pronostico_manana_tarde(datos, suma_manana, tipo_suma='Suma_Fijo'):
    """Dado una suma de la mañana, devuelve qué puede salir en la tarde."""
    resultados = Counter()
    
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'T' in sesiones:
            if sesiones['M'][tipo_suma] == suma_manana:
                resultados[sesiones['T'][tipo_suma]] += 1
    
    return resultados.most_common(10)

def obtener_pronostico_manana_noche(datos, suma_manana, tipo_suma='Suma_Fijo'):
    """Dado una suma de la mañana, devuelve qué puede salir en la noche (qué puede seguir)."""
    resultados = Counter()
    
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'N' in sesiones:
            if sesiones['M'][tipo_suma] == suma_manana:
...
def obtener_pronostico_manana_tarde_total(datos, suma_manana):
    """Dado una suma de la mañana, devuelve qué sumas totales pueden aparecer en la tarde."""
    resultados = Counter()
    
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'T' in sesiones:
            sumas_manana = obtener_sumas_totales_sesion(sesiones, 'M')
            if suma_manana in sumas_manana:
                for s in obtener_sumas_totales_sesion(sesiones, 'T'):
                    resultados[s] += 1
    
    return resultados.most_common(10)

def obtener_pronostico_manana_noche_total(datos, suma_manana):
    """Dado una suma de la mañana, devuelve qué sumas totales pueden aparecer en la noche."""
    resultados = Counter()
...
def obtener_pronostico_manana_tarde_noche_total(datos, suma_manana, suma_tarde):
    """Dado suma mañana y tarde (totales), devuelve qué puede salir en la noche."""
    resultados = Counter()
    ...
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'T' in sesiones and 'N' in sesiones:
            ...
            if suma_manana in sumas_manana and suma_tarde in sumas_tarde:
                for s in obtener_sumas_totales_sesion(sesiones, 'N'):
                    resultados[s] += 1
    
    return resultados.most_common(10)

def obtener_pronostico_dia_siguiente_total(datos, suma_m, suma_t, suma_n):
    """Dado suma mañana, tarde y noche (totales), devuelve qué puede salir en la mañana siguiente."""
    resultados = Counter()
    fechas_ordenadas = sorted(datos.keys())
    ...
    for i in range(len(fechas_ordenadas) - 1):
        ...
        if all(s in datos[fecha_actual] for s in ['M', 'T', 'N']) and 'M' in datos[fecha_siguiente]:
            sumas_m = obtener_sumas_totales_sesion(datos[fecha_actual], 'M')
            sumas_t = obtener_sumas_totales_sesion(datos[fecha_actual], 'T')
            sumas_n = obtener_sumas_totales_sesion(datos[fecha_actual], 'N')
            
            if suma_m in sumas_m and suma_t in sumas_t and suma_n in sumas_n:
                for s in obtener_sumas_totales_sesion(datos[fecha_siguiente], 'M'):
                    resultados[s] += 1
    
    return resultados.most_common(10)

def obtener_pronostico_manana_tarde_noche(datos, suma_manana, suma_tarde, tipo_suma='Suma_Fijo'):
    """Dado suma mañana y tarde, devuelve qué puede salir en la noche."""
    resultados = Counter()
    
    for fecha, sesiones in datos.items():
        if 'M' in sesiones and 'T' in sesiones and 'N' in sesiones:
            if sesiones['M'][tipo_suma] == suma_manana and sesiones['T'][tipo_suma] == suma_tarde:
                resultados[sesiones['N'][tipo_suma]] += 1
    
    return resultados.most_common(10)

def obtener_pronostico_dia_siguiente(datos, suma_m, suma_t, suma_n, tipo_suma='Suma_Fijo'):
    """Dado suma mañana, tarde y noche, devuelve qué puede salir en la mañana siguiente."""
    resultados = Counter()
    fechas_ordenadas = sorted(datos.keys())
    ...
    for i in range(len(fechas_ordenadas) - 1):
        ...
        if all(s in datos[fecha_actual] for s in ['M', 'T', 'N']) and 'M' in datos[fecha_siguiente]:
            if (datos[fecha_actual]['M'][tipo_suma] == suma_m and 
                datos[fecha_actual]['T'][tipo_suma] == suma_t and 
                datos[fecha_actual]['N'][tipo_suma] == suma_n):
                resultados[datos[fecha_siguiente]['M'][tipo_suma]] += 1
...
def main():
    ...
    **Orden en historial:** N → T → M (de más reciente a menos reciente)
    """)
    ...
    with tab2:
        ...
        st.markdown("""
        **Análisis de transiciones entre sesiones:**
        - Si en la mañana sale suma X → ¿Qué puede salir en la tarde?
        - Si mañana=X y tarde=Y → ¿Qué puede salir en la noche?
        - Si mañana=X, tarde=Y, noche=Z → ¿Qué puede salir en la mañana siguiente?
        """)
        
        # Preparar datos
        datos = preparar_datos_transiciones(df)
        
        # Seleccionar tipo de suma a analizar
        ...
        # === SECCIÓN 1: MAÑANA → TARDE ===
        st.subheader("🌅 Si en la Mañana sale suma X → ¿Qué puede salir en la Tarde?")
        
        col1, col2 = st.columns([1, 2])
        with col1:
            suma_manana_input = st.selectbox("Suma de la Mañana:", list(range(19)), key="comb_manana")
        
        if st.button("Analizar Mañana → Tarde", key="btn_comb_mt"):
            if tipo_suma_sel == 'Suma_Total':
                pronostico = obtener_pronostico_manana_tarde_total(datos, suma_manana_input)
            else:
                pronostico = obtener_pronostico_manana_tarde(datos, suma_manana_input, tipo_suma_sel)
            
            if pronostico:
                df_pron = pd.DataFrame([
                    {'Suma Tarde': s, 'Frecuencia': f} for s, f in pronostico
                ])
                st.success(f"Si Mañana = Σ{suma_manana_input}, lo más probable para Tarde:")
                st.dataframe(df_pron, use_container_width=True, hide_index=True)
            else:
                st.warning(f"No hay datos históricos con Mañana = Σ{suma_manana_input}")
        
        st.markdown("---")
        
        # === SECCIÓN 2: MAÑANA → NOCHE (QUÉ PUEDE SEGUIR) ===
        st.subheader("🌙 Si en la Mañana sale suma X → ¿Qué puede seguir en la Noche?")
        
        col1, col2 = st.columns([1, 2])
        with col1:
            suma_manana_noch = st.selectbox("Suma de la Mañana:", list(range(19)), key="comb_manana_noch")
        
        if st.button("Analizar Mañana → Noche (qué puede seguir)", key="btn_comb_mn"):
            if tipo_suma_sel == 'Suma_Total':
                pronostico = obtener_pronostico_manana_noche_total(datos, suma_manana_noch)
            else:
                pronostico = obtener_pronostico_manana_noche(datos, suma_manana_noch, tipo_suma_sel)
            
            if pronostico:
                df_pron = pd.DataFrame([
                    {'Suma Noche': s, 'Frecuencia': f} for s, f in pronostico
                ])
                st.success(f"Si Mañana = Σ{suma_manana_noch}, lo más probable para Noche (qué puede seguir):")
                st.dataframe(df_pron, use_container_width=True, hide_index=True)
            else:
                st.warning(f"No hay datos históricos con Mañana = Σ{suma_manana_noch}")
        
        st.markdown("---")
        
        # === SECCIÓN 3: MAÑANA + TARDE → NOCHE ===
        st.subheader("🌙 Si Mañana=X y Tarde=Y → ¿Qué puede salir en la Noche?")
        
        col1, col2, col3 = st.columns([1, 1, 1])
        with col1:
            suma_m_input = st.selectbox("Suma Mañana:", list(range(19)), key="comb_m2")
        with col2:
            suma_t_input = st.selectbox("Suma Tarde:", list(range(19)), key="comb_t2")
        
        if st.button("Analizar Mañana+Tarde → Noche", key="btn_comb_mtn"):
            if tipo_suma_sel == 'Suma_Total':
                pronostico = obtener_pronostico_manana_tarde_noche_total(datos, suma_m_input, suma_t_input)
            else:
                pronostico = obtener_pronostico_manana_tarde_noche(datos, suma_m_input, suma_t_input, tipo_suma_sel)
            
            if pronostico:
                df_pron = pd.DataFrame([
                    {'Suma Noche': s, 'Frecuencia': f} for s, f in pronostico
                ])
                st.success(f"Si Mañana=Σ{suma_m_input} y Tarde=Σ{suma_t_input}, lo más probable para Noche:")
                st.dataframe(df_pron, use_container_width=True, hide_index=True)
            else:
                st.warning(f"No hay datos históricos con Mañana=Σ{suma_m_input} y Tarde=Σ{suma_t_input}")
        
        st.markdown("---")
        
        # === SECCIÓN 4: DÍA COMPLETO → MAÑANA SIGUIENTE ===
        st.subheader("📅 Si Mañana=X, Tarde=Y, Noche=Z → ¿Qué puede salir en la Mañana siguiente?")
        
        col1, col2, col3 = st.columns([1, 1, 1])
        with col1:
            suma_m3 = st.selectbox("Suma Mañana:", list(range(19)), key="comb_m3")
        with col2:
            suma_t3 = st.selectbox("Suma Tarde:", list(range(19)), key="comb_t3")
        with col3:
            suma_n3 = st.selectbox("Suma Noche:", list(range(19)), key="comb_n3")
        
        if st.button("Analizar Día Completo → Mañana Siguiente", key="btn_comb_dia"):
            if tipo_suma_sel == 'Suma_Total':
                pronostico = obtener_pronostico_dia_siguiente_total(datos, suma_m3, suma_t3, suma_n3)
            else:
                pronostico = obtener_pronostico_dia_siguiente(datos, suma_m3, suma_t3, suma_n3, tipo_suma_sel)
            
            if pronostico:
                df_pron = pd.DataFrame([
                    {'Suma Mañana Sig.': s, 'Frecuencia': f} for s, f in pronostico
                ])
                st.success(f"Si Mañana=Σ{suma_m3}, Tarde=Σ{suma_t3}, Noche=Σ{suma_n3}, lo más probable para la Mañana siguiente:")
                st.dataframe(df_pron, use_container_width=True, hide_index=True)
            else:
        ...
        # Top transiciones Mañana → Tarde
        if tipo_suma_sel != 'Suma_Total':
            trans_mt = analizar_transicion_manana_tarde(datos, tipo_suma_sel)
            top_mt = trans_mt.most_common(15)
            
            if top_mt:
                st.markdown("**Top 15 transiciones Mañana → Tarde:**")
                df_trans_mt = pd.DataFrame([
                    {'Mañana': f"Σ{m}", 'Tarde': f"Σ{t}", 'Frecuencia': f} 
                    for (m, t), f in top_mt
                ])
                st.dataframe(df_trans_mt, use_container_width=True, hide_index=True)
        else:
        ...
        if st.button("Generar Almanaque", key="btn_alm_total"):
            ...
            if not resultado['success']:
                ...
                if not resultado['df_historial_actual'].empty:
                    st.subheader("📋 Historial Período Actual (Orden: N → T → M)")
    ...
    with tab6:
        ...
        if st.button("Buscar Suma", key="btn_buscar"):
            ...
            if df_resultado is None or df_resultado.empty:
                ...
                st.subheader(f"📋 Historial de Apariciones (Orden: N → T → M)")
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("*SumaDigitos v5.1*")
